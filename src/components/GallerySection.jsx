...
function GallerySection({ images, siteImages = [], backgroundImage }) {
  ...
  const galleryModal = selectedImage ? (
    <div
      className="fixed inset-0 z-[9999] flex min-h-dvh items-center justify-center overflow-y-auto bg-[#222222]/82 p-3 backdrop-blur-sm sm:p-6"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className="relative my-auto w-full max-w-6xl overflow-hidden rounded-[1.5rem] border border-[#FFE0CC] bg-white/98 p-2 shadow-[0_30px_90px_rgba(0,0,0,0.36)] sm:rounded-[2rem] sm:p-3"
        onClick={(event) => event.stopPropagation()}
      >
        <button
          type="button"
          onClick={() => setSelectedImage(null)}
          className="absolute right-3 top-3 z-30 grid size-11 place-items-center rounded-full bg-white text-[#222222] shadow-[0_12px_32px_rgba(11,16,38,0.18)] transition hover:bg-[#FFF1E8] hover:text-[#FF6A2A] sm:right-4 sm:top-4"
          aria-label="Galeri görselini kapat"
        >
          <X size={20} aria-hidden="true" />
        </button>
        <div className="grid max-h-[72dvh] place-items-center overflow-hidden rounded-[1.15rem] bg-[#FFF8F0] sm:max-h-[78dvh] sm:rounded-[1.5rem]">
          <img
            src={selectedImage.image_url}
            alt={selectedImage.alt_text || selectedImage.alt || selectedImage.title || 'Orion Kamp galeri görseli'}
            className="max-h-[72dvh] w-auto max-w-full object-contain sm:max-h-[78dvh]"
          />
        </div>
  ...
  return (
    <>
      ...
      {galleryModal ? createPortal(galleryModal, document.body) : null}
    </>
  )
}
